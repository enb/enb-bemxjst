 ...
 * @param {String}   options.tech                   bemtree or bemhtml.
...
exports.compile = function (code, options) {
    ...
    var exportName = options.exportName || 'BEMHTML',
        ...
        vowRequireDecl = requires['vow'] || requires['Vow'],
        libs = {
    ...
    return compileCommonJSRequire(requires, options.dirname).then(function (commonJSRequire) {
        return [
            ...
            tech === 'bemtree' ? (options.includeVow ? vowCode :
                ('var Vow = __bem_xjst_libs__.vow || __bem_xjst_libs__.Vow;' + EOL +
                'if (!Vow) throw new Error("Vow is not defined");')) : '',
            code,
            '    return exports;',
            '};',
            commonJSRequire,
            ...
            // We should not try to build bemtree if there is no vow
            Boolean(tech !== 'bemtree' || vowRequireDecl && (vowRequireDecl.commonJS || vowRequireDecl.global)) + ') {',
            ...
            'if(defineAsGlobal) {',
            '    ' + exportName + ' = buildBemXjst({}, ' + libs.global + ');',
